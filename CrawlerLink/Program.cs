...
namespace CrawlerLink
{
    class Program
    {
        ...
        private static void EndCawler()
        {
            ...
            {
                channel.QueueDeclare(queue: "amqueuehelloworld",
                durable: true,
                exclusive: false,
                autoDelete: false,
                arguments: null);
                ...
                consumer.Received += async (model, ea) =>
                {
                    ...
                    if (!dbContextCrawler.Articles.Any(o => o.Title == crawler.Title))
                    {
                        dbContextCrawler.Articles.Add(crawler);
                        dbContextCrawler.SaveChanges();
                    }

                };
                channel.BasicConsume(queue: "amqueuehelloworld",
                autoAck: true,
                consumer: consumer);
